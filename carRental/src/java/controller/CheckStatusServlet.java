...
public class CheckStatusServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            boolean b = x.checkRecord(request.getParameter("id"));
            if(b)
            {
                Manage m = new Manage();
                m = x.checkRecord2(request.getParameter("id"));
                if(m.getStatus().equals("approved"))
                {
                    request.setAttribute("place", m.getPlace());
                  rd = request.getRequestDispatcher("JSP/success6.jsp");  
                }
                else
                {
